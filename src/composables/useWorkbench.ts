...
export type CardId = "chart" | "order" | "watch" | "rank";
...
// 窄卡片在工作台上的纵向排列顺序（盘口优先靠上）
const NARROW_ORDER: CardId[] = ["order", "watch", "rank"];

// 模式预设：一键切换一整套卡片
export const MODES: Record<string, CardId[]> = {
  pro: ["chart", "order", "watch"],
  scanner: ["rank", "watch"],
  full: ["chart", "order", "watch", "rank"],
  chart: ["chart"],
};
